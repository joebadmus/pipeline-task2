pipeline {
    ...
    stages {
        ...
        stage ("Provision App Server") {
            steps {
                    withCredentials([usernamePassword(credentialsId: 'AWS_DEV_SECRET', passwordVariable: 'my_aws_secret', usernameVariable: 'my_aws_key')]) {
                    sh  'cd app-server/infra/ && AWS_ACCESS_KEY_ID=$my_aws_key AWS_SECRET_ACCESS_KEY=$my_aws_secret terraform detroy --auto-approve'
                    }
                    echo '====++++App server setup complete++++===='
            }
        }
        ...
        stage ("Provision Web Server") {
            steps {
                    withCredentials([usernamePassword(credentialsId: 'AWS_DEV_SECRET', passwordVariable: 'my_aws_secret', usernameVariable: 'my_aws_key')]) {
                    sh  'cd web-server/infra/ && AWS_ACCESS_KEY_ID=$my_aws_key AWS_SECRET_ACCESS_KEY=$my_aws_secret terraform detroy --auto-approve'
                    }
                    echo '====++++Web server setup complete++++===='
            }
        }
